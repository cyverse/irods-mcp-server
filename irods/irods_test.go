...
func TestTool(t *testing.T) {
	...
	if mcpConfig.Config.Username != "anonymous" {
		t.Run("ListTickets", testListTickets)
		t.Run("GetTicketInfo", testGetTicketInfo)
	}
}
...
func testListTickets(t *testing.T) {
	myTool := mcpServer.GetTool(ListTicketsName)
	assert.NotNil(t, myTool)
	...
	assert.Contains(t, result.Content[0].Type, "text")
	assert.Contains(t, result.Content[0].Text, "\"ticket\":")
}

func testGetTicketInfo(t *testing.T) {
	myTool := mcpServer.GetTool(GetTicketInfoName)
	assert.NotNil(t, myTool)
